...
try:
    DEFAULT_EMBEDDER = SentenceTransformerEmbedding()
    print("✅ 使用真实嵌入模型: SentenceTransformer(all-MiniLM-L6-v2)")
except ImportError:
    warnings.warn(
        "sentence-transformers 未安装，降级使用 HashEmbedding（仅用于测试，不支持语义检索）。"
